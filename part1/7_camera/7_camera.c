...
void lookAt(vec3s position, vec3s target, vec3s up, mat4 view);
...
int main(void) {
    ...
    GLFWwindow *window = glfwCreateWindow(screen_w, screen_h, "7 Camera", NULL, NULL);
    if (window == NULL) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        {
            const float r = 10.0f;
            camera.position.x = r * sin(time);
            camera.position.z = r * cos(time);
            lookAt(camera.position, camera.target, (vec3s){0.0f, 1.0f, 0.0f}, view);
            /* glm_lookat(camera.position.raw, camera.target.raw, (vec3s){0.0f, 1.0f, 0.0f}.raw, view); */
...
void lookAt(vec3s position, vec3s target, vec3s up, mat4 view) {
    vec3s camera_direction_reverse = glms_vec3_sub(position, target);
    glm_normalize(camera_direction_reverse.raw);

    vec3s camera_right = glms_cross((vec3s){0.0f, 1.0f, 0.0f}, camera_direction_reverse);
    glm_normalize(camera_right.raw);

    vec3s camera_up = glms_cross(camera_direction_reverse, camera_right);

    const vec3s R = camera_right;
    const vec3s U = camera_up;
    const vec3s D = camera_direction_reverse;
    const vec3s P = position;

    const float RdotP = R.x * P.x + R.y * P.y + R.z * P.z;
    const float UdotP = U.x * P.x + U.y * P.y + U.z * P.z;
    const float DdotP = D.x * P.x + D.y * P.y + D.z * P.z;
    ...
    glm_mat4_transpose(lookat);
    glm_mat4_copy(lookat, view);
}
